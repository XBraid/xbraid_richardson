...
typedef struct _braid_App_struct
{
   ...
   int       solver;        /* solver = 1 for BE and 0 for SDIRK-2 */
   int       richardson;
   int       cfactor;
...
int sequential_solver(braid_App    app)
{
   ...
   deltaX = (app->xstop - app->xstart) / (u->size - 1.0);
   ...
   if ( !(app->richardson) )
   {
      ...
      {
         tstart += deltaT;
         take_step( u->values, u->size, tstart, app->xstart, deltaX, deltaT, app->matrix, app->g); 
   ...
   else
   {
      ...
      do 
      {
         ...
         take_step( u_cpy->values, u_cpy->size, tstart, app->xstart, deltaX, cfactor*deltaT, app->matrix, app->g);
         ...
         for ( i = 0; i < u->size  ; i++ )
         {
            u->values[i] = a*u->values[i] - b*u_cpy->values[i] ;
            u_cpy->values[i] = u->values[i];
         }
      } while ( tstop - tstart > 1e-10 );
   }
   double error = compute_error_norm(u->values, app->xstart, app->xstop, u->size, tstop);
   printf("  Discretization error at final time:  %1.4e\n", error);
...
int main (int argc, char *argv[])
{
   ...
   while (arg_index < argc)
   {
      if ( strcmp(argv[arg_index], "-help") == 0 )
      {
         if ( rank == 0 )
         {
            ...
            printf(" Run with \n");
            printf(" $$ ./ex-02-richardson -richardson \n");
            printf(" OR \n");
            printf(" $$./ex-02-richardson\n\n");
            printf(" Note how the discretization error changes between the two options.\n");
            printf(" Need to increase spatial resolution to see effect of Richardson.\n"); 
            printf(" Jacob:  I actually had a hard time seeing a benefit of Richardson here.\n"); 
         }
         exit(1);
      }
   ...
   else
   {
      ...
      braid_SetErrorEstimation(core, 0,richardson,2);
