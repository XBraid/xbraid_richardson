          ...
          braid_Real dt)
{
   ...
   if (solver)
      return myBE(x,tstart,dt);
   else
      return mysdirk23(x,tstart,dt);
}
        ...
        braid_StepStatus status)
{
   ...
   u->value0 = my_solver(app->solver, u->value0, tstart, dt );
          ...
          braid_AccessStatus astatus)
{
   ...
   if (done && t == app->tstop ) 
      printf("Disretization error (t,sol,err): %.20f %.20f %.20f \n", t, u->value0, fabs( (u->value0)- exact_solution )  );
   ...
   while (arg_index < argc)
   {
      if ( strcmp(argv[arg_index], "-help") == 0 )
      {
         ...
         if ( myid == 0 )
         {
            ...
            printf("  -solver           : use sdirk23 (default Backward Euler) \n");
  
            printf("e.g. ./ex-07 -> BE with no richardson \n");
            printf("e.g. ./ex-07 -solver -> SDIRK23 with no richardson \n");
            printf("e.g. ./ex-07 -richardson -> BE with richardson \n");
            printf("e.g. ./ex-07 -solver -richardson -> SDIRK23 with richardson \n");
         }
         exit(1);
      }
