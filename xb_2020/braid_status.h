 ...
 * through _braid_BufferStatusGet**()_ functions. This is just a pointer to the braid_Status.
 */
typedef struct _braid_GlobalAccessStatus_struct *braid_GlobalAccessStatus;
...
braid_StatusGetCallingFunction(braid_Status status,        /**< structure containing current simulation info */
                               braid_Int   *cfunction_ptr  /**< output, function number (see below) */
                               );
...
/** 
 * Return a pointer to the start of the local RFactors array 
 **/
braid_Int
braid_StatusGetRFactors(braid_Status   status,              /**< structure containing current simulation info */
                        braid_Int     *npoints,             /**< output, number of points in rfactor array */
                        braid_Int    **rfactors             /**< output, pointer to the start of rfactors array */
                        );

/** 
 * Return the WFactor for the corresponding Vector.  
 */
braid_Int
braid_StatusGetWFactor(braid_Status  status,
                       braid_Real   *wfactor);

/**
 * Return a pointer to the start of the local WFactors array 
 */
braid_Int
braid_StatusSetWFactor(braid_Status status,
                       braid_Real   wfactor
                       );

/** 
 * Return a pointer to the start of the local time grid 
 **/
braid_Int
braid_StatusGetTGrid(braid_Status status,                /**< structure containing current simulation info */
                     braid_Int    *npoints,              /**< output, number of points in the array */            
                     braid_Real   **tgrid                /**< output, pointer to the begining of local time array */
                     );

/** 
 * Return the cfactor on the finest grid 
 **/
braid_Int
braid_StatusGetCFactor(braid_Status status,              /**< structure containing current simulation info */
                       braid_Int    *cfactors            /**< output, cfactor on level 0  */           
                      );

/** 
 * Return the local ilower, iupper and gupper values on 
 * the finest grid 
 **/
braid_Int 
braid_StatusGetInterval(braid_Status status,             /**< structure containing current simulation info */
                        braid_Int    *ilower,            /**< output, local ilower on level 0*/        
                        braid_Int    *iupper,            /**< output, local iupper on level 0*/
                        braid_Int    *gupper             /**< output, global gupper on level 0 */
                        );

/* Set done in the core, so that XBraid stops iterating at the end of the
 * current V cycle 
 */
braid_Int
braid_StatusSetRefine(braid_Status status,                 /**< structure containing current simulation info */
                    braid_Int    done                    /**< input, Tell XBraid to quit iterating */        
                     );                                 
...
ACCESSOR_HEADER_GET1(Access, WFactor,         Real)
...
ACCESSOR_HEADER_GET1(CoarsenRef, CallingFunction, Int)
...
ACCESSOR_HEADER_SET1(Step, WFactor,       Real)
...
ACCESSOR_HEADER_GET1(Buffer, CallingFunction, Int)
/*--------------------------------------------------------------------------
 * GlobalAccessStatus Prototypes: They just wrap the corresponding Status accessors
 *--------------------------------------------------------------------------*/

ACCESSOR_HEADER_GET2(GlobalAccess, RFactors, Int, Int *)
ACCESSOR_HEADER_GET2(GlobalAccess, ErrorEst, Int, Real *)
ACCESSOR_HEADER_GET2(GlobalAccess, TGrid,    Int, Real *)
ACCESSOR_HEADER_GET1(GlobalAccess, CFactor,  Int)
ACCESSOR_HEADER_GET3(GlobalAccess, Interval, Int, Int,Int)
ACCESSOR_HEADER_GET1(GlobalAccess, Iter,     Int)
ACCESSOR_HEADER_GET1(GlobalAccess, NRefine,  Int)
ACCESSOR_HEADER_GET2(GlobalAccess, RNorms,   Int, Real)
ACCESSOR_HEADER_GET1(GlobalAccess, CallingFunction, Int)
ACCESSOR_HEADER_SET1(GlobalAccess, Refine,   Int)
ACCESSOR_HEADER_SET1(GlobalAccess, RSpace,   Int)
...
ACCESSOR_HEADER_GET2(GlobalAccess, RFactors, Int, Int *)
ACCESSOR_HEADER_GET2(GlobalAccess, WFactors, Int, Real *)
ACCESSOR_HEADER_GET2(GlobalAccess, TGrid,    Int, Real *)
ACCESSOR_HEADER_GET1(GlobalAccess, CFactor,  Int)
ACCESSOR_HEADER_GET3(GlobalAccess, Interval, Int, Int,Int)
ACCESSOR_HEADER_GET1(GlobalAccess, Iter,     Int)
ACCESSOR_HEADER_GET1(GlobalAccess, NRefine,  Int)
ACCESSOR_HEADER_GET2(GlobalAccess, RNorms,   Int, Real)
ACCESSOR_HEADER_SET1(GlobalAccess, Done,     Int)
ACCESSOR_HEADER_SET1(GlobalAccess, RSpace,   Int)
...
#define braid_Caller_FTest     -1
/** When CallingFunction equals 0, Braid is in FInterp */
#define braid_Caller_FInterp   0
/** When CallingFunction equals 0, Braid is in FRestrict */
#define braid_Caller_FRestrict 1
/** When CallingFunction equals 0, Braid is in FRefine */
#define braid_Caller_FRefine   2
/** When CallingFunction equals 0, Braid is in FAccess */
#define braid_Caller_FAccess   3
/** When CallingFunction equals 4, Braid is in FCRelaxation */
#define braid_Caller_FCRelax   4 
/** When CallingFunction equals 5, Braid is in GlobalAccess */
#define braid_Caller_GlobalAccess   5
/** When CallingFunction equals 6, Braid is in InitGuess */
#define braid_Caller_InitGuess 6
/** When CallingFunction equals 7, Braid is in ComputeFullRNorm */
#define braid_Caller_ComputeFullRNorm 7
