...
public:
   ...
   */
   ...
   {
      fprintf(stderr, "Braid C++ Wrapper Warning: turn off Residual until"
                      "until Residual has been user implemented\n");
      return 0;
   }
   ...
   virtual braid_Int GlobalAccess(MPI_Comm            comm,
                                  braid_Vector       *u_ptr,
                                  braid_Int          *umask,
                                  braid_Int           npoints, 
                                  BraidGAccessStatus &status)
   {
      int id; 
      MPI_Comm_rank(comm,&id);
      if (id == 0)
         fprintf(stderr, "Braid C++ Wrapper Warning GlobalAccess not implimented \n");
      return 0;
   }
...
class BraidAccessStatus
{
   ...
   public:
      ...
      void GetCallingFunction(braid_Int *caller_ptr) { braid_AccessStatusGetCallingFunction(astatus, caller_ptr); }
...
class BraidGAccessStatus 
{
   ...
   public:
      BraidGAccessStatus(braid_GlobalAccessStatus _gstatus)
      {
         gstatus = _gstatus;
      }
      void GetRFactors(braid_Int *npoints, braid_Int **rfactors){ braid_GlobalAccessStatusGetRFactors(gstatus, npoints, rfactors);}
      void GetErrorEst(braid_Int *npoints, braid_Real **error_est){ braid_GlobalAccessStatusGetErrorEst(gstatus, npoints, error_est);}
      void GetTGrid(braid_Int *npoints, braid_Real **tgrid){ braid_GlobalAccessStatusGetTGrid(gstatus, npoints, tgrid);}
      void GetCFactor(braid_Int *cfactor){ braid_GlobalAccessStatusGetCFactor(gstatus, cfactor);}
      void GetInterval(braid_Int *ilower, braid_Int *iupper, braid_Int *gupper){braid_GlobalAccessStatusGetInterval(gstatus,ilower,iupper,gupper);}
      void GetIter(braid_Int *iter)         { braid_GlobalAccessStatusGetIter(gstatus, iter); }
      void GetNRefine(braid_Int *nrefine)   { braid_GlobalAccessStatusGetNRefine(gstatus, nrefine); }
      void SetRefine(braid_Int refine)      { braid_GlobalAccessStatusSetRefine(gstatus, refine);}
      void SetRSpace(braid_Int rspace)      { braid_GlobalAccessStatusSetRSpace(gstatus, rspace); } 
      void GetCallingFunction(braid_Int *caller_ptr) { braid_GlobalAccessStatusGetCallingFunction(gstatus, caller_ptr); }
      ~BraidGAccessStatus() { }
};
...
class BraidStepStatus
{
   ...
   public:
      ...
      void GetCallingFunction(braid_Int *caller_ptr)              { braid_StepStatusGetCallingFunction(pstatus, caller_ptr); }
...
class BraidCoarsenRefStatus
{
   ...
   public:
      ...
      void GetCallingFunction(braid_Int *caller_ptr)     { braid_CoarsenRefStatusGetCallingFunction(cstatus, caller_ptr); }
...
class BraidBufferStatus
{
   ...
   public:
      ...
      void GetCallingFunction(braid_Int *caller_ptr)             { braid_BufferStatusGetCallingFunction(bstatus, caller_ptr); }
      ~BraidBufferStatus() {} 
};
...
static braid_Int _BraidAppGAccess(braid_App               _app,
                                  MPI_Comm               comm,
                                  braid_Vector         *u_ptr,
                                  braid_Int            *umask,
                                  braid_Int             npoints,
                                  braid_GlobalAccessStatus  _gstatus)
{
   BraidApp *app = (BraidApp*)_app;
   BraidGAccessStatus gstatus(_gstatus);
   return app -> GlobalAccess(comm,u_ptr,umask,npoints, gstatus);
}
...
public:
   ...
   void SetGlobalAccess() { braid_SetGlobalAccess(core, _BraidAppGAccess); }
