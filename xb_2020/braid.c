   ...
   while (!done)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            refined = 0;
            _braid_CoreElt(core, refine_done) = 1;
            _braid_GlobalAccess(core);
            if ( _braid_CoreElt(core,refine_done) == 1 ) //Refine in time 
            {
            ...
            else if ( _braid_CoreElt(core,refine_done) == -1 ) // No refine, and allow xbraid to finish
            {
                 /* Check convergence */
                 _braid_DrivePrintStatus(core, level, iter, refined, localtime );
                 _braid_DriveCheckConvergence(core, iter, &done);
            ...
            else // No refinement completed, but not ready to finish yet 
            {
                 _braid_DriveCheckConvergence(core, iter, &done);
                 _braid_DrivePrintStatus(core, level, iter, refined, localtime );
                  iter++;
           ...
           braid_Core            *core_ptr)
{
   ...
   braid_Int              comm_min        = 1;              /* Default comm level */
   ...
   _braid_CoreElt(core, comm_min)        = comm_min;
   ...
   _braid_CoreElt(core, rfactor)         = 1;
   _braid_CoreElt(core, wfactor)         = 1; /*BO: just incase the user doesnt set it*/
...
braid_Int
braid_SetCommMin(braid_Core  core,
                 braid_Int   comm_min)
{
   _braid_CoreElt(core, comm_min) = comm_min;
...
braid_Int
braid_SetGlobalAccess(braid_Core              core,
                      braid_PtFcnGlobalAccess gaccess
                  )
{
   _braid_CoreElt(core, gaccess) = gaccess;
