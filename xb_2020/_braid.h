...
typedef struct _braid_Core_struct
{
   ...
   braid_PtFcnGlobalAccess   gaccess;          /**< (optional) set a global access function to be called before FRefine */

   braid_Int              comm_min;         /**< boolean, if set, first point on all processors is a C-point */
   ...
   braid_Real    wfactor;          /**< if set by user, allows for weighted load balancing*/
...
/**
 * Accessor for _braid_BalanceStruct attibutes
 **/
#define _braid_BalanceElt( balance, elt)  ( (balance) -> elt )
...
/**
 * Sets the owner of ilower - 1 and iupper + 1 on all levels using the
 * BlockDist functions.
 */
braid_Int
 ...
 * the left ( -1 ) or right ( 1 ) on level l.
...
/**
 * Propergate time step information required to compute the error estimate
...
_braid_GetFullRNorm(braid_Core  core,
                    ...
                    braid_Real *rnorm_ptr);
/**
 * Allow the user global access following a V cycle, but before FRefine 
 */
