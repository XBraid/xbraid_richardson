...
/**
 * Gives user access to XBraid following a MGRIT cycle, and prior
 * to temporal refinement. This allows the user to process the rfactors,
 * wfactors, and solution vectors, prior to temporal refinement and load 
 * balancing. Moreover, the user can access an array of pointers to the 
 * current solution. If the user deems the solution accurate enough, then 
 * done can be set in the GAccessStatus struct, to tell XBraid to stop iterating
 * umask[i] == 0 if solution is stored at index i, -1 if a shell vector at i, and 
 * -2 if nothing is stored at index i **/

typedef braid_Int
(*braid_PtFcnGlobalAccess)(braid_App                app,              /**< user-defined _braid_App structure */
                           MPI_Comm                 comm,             /**< Time communicator */
                           braid_Vector            *u_ptr,            /**< pointer to the start of the local solution array */
                           braid_Int               *umask,            /**< mask specifiying status of solutions in u */
                           braid_Int                npoints,          /**< number of points in the u_ptr array */
                           braid_GlobalAccessStatus status            /**< can be querried for info like length of the solution array */
                          );
/** @}*/
...
/** 
 * Distribute time points based on C points. There is nothing to be 
 * gained by splitting F intervals across temporal processors. comm_min = 1, 
 * turns on C-interval distribution. 
 **/
braid_Int
braid_SetCommMin(braid_Core  core,    /**< braid_Core (_braid_Core) struct*/
                 braid_Int   comm_min /**< boolean, balance base on C intervals or not */
                );
 ...
 **/
braid_Int
braid_SetGlobalAccess(braid_Core          core,        /**< braid_Core (_braid_Core) struct*/
                      braid_PtFcnGlobalAccess gaccess  /**< function pointer to global access routine */
                     );
...
/**
 * Set Communication minimization on/off. If set, then the first point
 * on any interval is a C-point. Fine grid intervals are not split up
 * across processors. Turn off if memory is an issue.
 * * Default is level 1.
 **/
braid_Int
braid_SetCommMin(braid_Core  core,          /**< braid_Core (_braid_Core) struct*/
                 braid_Int   comm_min      /**< desired comm level */
                );
...
/**
 * Turn on error estimation and/or richardson extrapolation with XBraid. When richardson is turned on,
 * Richardson extrapolation (RE) is used to improve the accuracy of the solution at the C-points. When 
 * est_error is set, RE is used to estimate the local truncation error at each F-point. These estimates
 * can be accessed through the Access and GlobalAccess Functions. local_order (<1) represents the LOCAL 
 * order of the time integration scheme. e.g. local_order = 2 for Backward Euler.
 **/
